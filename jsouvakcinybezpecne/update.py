...
def get_vaccinations(timestamp):
    vaccines = pd.read_csv('https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/ockovani.csv', parse_dates=['datum'], infer_datetime_format=True)
...
def get_deaths(timestamp):
    deaths = pd.read_csv('https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/nakazeni-vyleceni-umrti-testy.csv', parse_dates=['datum'], infer_datetime_format=True)
...
def get_cases(timestamp):
    cases = pd.read_csv('https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/nakaza.csv', parse_dates=['datum'], infer_datetime_format=True)
